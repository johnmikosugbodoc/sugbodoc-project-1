<h3>Create task</h3>

<?php  
    $attributes = array('id' => 'create_form', 'class' => 'form_horizontal');
 ?>

<?php echo validation_errors("<p class='text-danger'>");?> <!-- ECHO ERRORS -->

<?php 
    echo form_open('tasks_controller/create_task/'.$this -> uri -> segment(3).'', $attributes);
 ?>
<form>
<div class="form-group mt-4"> <!--THIS IS task_NAME INPUT VALUE-->

<?php  
    echo form_label('Task Name'); 
 ?>
<?php  //to insert input type
    $data = array(

        'class' => 'form-control',
        'name' => 'task_name'
        
    );
 ?>
 <?php echo form_input($data);?>
</div>
<div class="form-group mt-4"> <!--THIS IS task_BODY INPUT VALUE-->

<?php  
    echo form_label('Task Description'); 
 ?>
<?php  //to insert input type
    $data = array(

        'class' => 'form-control',
        'name' => 'task_body'
        
    );
 ?>
 <?php echo form_textarea($data);?>
</div>
<div class="form-group mt-4"> <!--THIS IS task_DUEDATE INPUT VALUE-->

<?php  //to insert input type
    $data = array(

        'class' => 'form-control',
        'name' => 'due_date',
        'type' => 'date'
        
    );
 ?>
 <?php echo form_input($data);?>
</div>

<div class="form-group mt-4 text-center"> <!--THIS IS SUBMIT BUTTON-->

    <?php  //to save
        $data = array(

            'class' => 'btn btn-primary',
            'name' => 'submit',
            'value' => 'Save'
            
    );
?>
<?php echo form_submit($data);?>
</div>
</form>

 <?php echo form_close(); ?>